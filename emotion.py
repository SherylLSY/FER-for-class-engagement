...
for face in faces:
    ...
    if not(all(item == 'VERY_UNLIKELY' for item in sentiment) ):
        ...
        cv2.putText (img,string, (x,y+8), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,255), 2)
    else:
        print("Emotion -> Neutral" )
        cv2.putText (img, 'Neutral' , (x,y+8), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,255), 2)
...
cv2.waitKey(0)
cv2.imwrite('cut5-output-emo/output_1.jpg',img)